...
export function getFeeAmount(bitcoinAmount, feeBasisPoints) {
 ...
 */
export function deriveUnhardenedPublicKey(extendedPublicKey, bitcoinNetwork) {
 ...
 */
export function deriveUnhardenedKeyPairFromRootPrivateKey(rootPrivateKey, bitcoinNetwork, paymentType, accountIndex) {
 ...
 */
export function createTaprootMultisigPayment(unspendableDerivedPublicKey, publicKeyA, publicKeyB, bitcoinNetwork) {
 ...
 */
export async function getFeeRate(bitcoinBlockchainAPIFeeURL, feeRateMultiplier) {
 ...
 */
export async function getUTXOs(bitcoinNativeSegwitTransaction, bitcoinBlockchainAPIURL) {
    ...
    const modifiedUTXOs = await Promise.all(userUTXOs.map(async (utxo) => {
        return {
 ...
 */
export function getFeeRecipientAddressFromPublicKey(feePublicKey, bitcoinNetwork) {
    const feePublicKeyBuffer = Buffer.from(feePublicKey, 'hex');
    const { address } = p2wpkh(feePublicKeyBuffer, bitcoinNetwork);
    if (!address)
        throw new Error('Could not create Fee Address from Public Key');
    return address;
}
/**
 ...
 */
export function getUnspendableKeyCommittedToUUID(vaultUUID, bitcoinNetwork) {
 ...
 * @param bitcoinNetwork - The Bitcoin Network to use.
 */
function getInputPaymentType(index, input, bitcoinNetwork) {
    const bitcoinAddress = getBitcoinInputAddress(index, input, bitcoinNetwork);
    if (bitcoinAddress === '')
        throw new Error('Bitcoin Address is empty');
 ...
 * @param bitcoinNetwork - The Bitcoin Network to use.
 */
function getBitcoinInputAddress(index, input, bitcoinNetwork) {
 ...
 */
export function createBitcoinInputSigningConfiguration(transaction, walletAccountIndex, bitcoinNetwork) {
    ...
    return indexesToSign.map(inputIndex => {
        ...
        if (isUndefined(input.index))
            throw new Error('Input must have an index for payment type');
 ...
 * @param bitcoinNetwork - The Bitcoin Network to use.
 */
export function getInputByPaymentTypeArray(signingConfiguration, psbt, bitcoinNetwork) {
...
export function getValueMatchingInputFromTransaction(bitcoinTransaction, bitcoinValue) {
    const valueMatchingTransactionInput = bitcoinTransaction.vout.find(output => output.value === bitcoinValue);
...
export function validateScript(script, outputScript) {
    return (outputScript.length === script.length &&
        outputScript.every((value, index) => value === script[index]));
}
export function finalizeUserInputs(transaction, userPayment) {
    ...
    createRangeFromLength(transaction.inputsLength).forEach(index => {
        ...
        if (inputScript.length === userPaymentScript.length &&
 ...
 */
export function ecdsaPublicKeyToSchnorr(publicKey) {
